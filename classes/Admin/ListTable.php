...
class ListTable extends WP_List_Table {
	...
	public function column_title( $item ): string {
		$title   = ! empty( $item['title'] ) ? $item['title'] : __( 'Untitled', 'buttons' );
	...
	public function get_columns(): array {
		return [
			...
			'title'  => __( 'Title', 'buttons' ),
			'code'   => __( 'Shortcode', 'buttons' ),
			'tag'    => __( 'Tag', 'buttons' ),
			'mode'   => __( 'Test mode',
					'buttons' ) . '<sup class="has-tooltip" data-tooltip="' . __( 'The item will only be displayed for administrators.',
					'buttons' ) . '">ℹ</sup>',
			'status' => __( 'Status',
					'buttons' ) . '<sup class="has-tooltip" data-tooltip="' . __( 'The item will only be displayed for administrators.',
					'buttons' ) . '">ℹ</sup>',
	...
	private function table_data(): array {
		...
		foreach ( $result as $key => $value ) {
			$title       = ! empty( $value->title ) ? $value->title : __( 'UnTitle', 'buttons' );
			$tooltip_off = esc_attr__( 'Click for Deactivate.', 'buttons' );
			$tooltip_on  = esc_attr__( 'Click for Activate.', 'buttons' );
			...
			$mode_tooltip_off = esc_attr__( 'Click for OFF.', 'buttons' );
			$mode_tooltip_on  = esc_attr__( 'Click for ON.', 'buttons' );
	...
	public function get_bulk_actions(): array {
		$actions = [
			'delete'     => __( 'Delate', 'buttons' ),
			'activate'   => __( 'Activate', 'buttons' ),
			'deactivate' => __( 'Deactivate', 'buttons' ),
			'test_on'    => __( 'Test mode ON', 'buttons' ),
			'test_off'   => __( 'Test mode OFF', 'buttons' ),
		];
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' === $which ) {
			...
			echo '</select>';
			submit_button( __( 'Filter', 'buttons' ), 'secondary', 'action', false );
			echo '</div>';
		}
	}
